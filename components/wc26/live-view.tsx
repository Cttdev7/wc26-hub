...
// LIVE VIEW — real-time match experience
// Ported 1:1 from design/js/13-live-view.jsx

import { useState, useEffect, useRef } from 'react'
import { TEAMS, LINEUPS, PREDICTORS } from './data'
...
const CHAT_SEED = [
  { who:'@northtactic',  txt:'On y est. Allez les Bleus !!', color:'#6B2FB5', m:'9'  },
  ...
  { who:'@maradona_22',  txt:'GOOOAAAAL MBAPPE !!! 🇫🇷',     color:'#FF0080', m:'23' },
  { who:'@xg_addict',    txt:'xG France 0.74 vs Brésil 0.51, mérité', color:'#0A0A0A', m:'31' },
  { who:'@bigben.bets',  txt:'Allez allez Bresil, faut égaliser', color:'#FF6E00', m:'38' },
  { who:'@tiki.taka',    txt:'Possession brésilienne énorme là', color:'#FFD400', m:'40' },
  { who:'@samba_pro',    txt:'VINI VINI VINI !!! 1-1 ⚽',     color:'#C8FF00', m:'41' },
  { who:'@maradona_22',  txt:'On va à la mi-temps là',       color:'#FF0080', m:'45' },
  { who:'@northtactic',  txt:'Deschamps doit recadrer le pressing', color:'#6B2FB5', m:'46' },
  { who:'@kylian.fr',    txt:'Cherki rentre, joli choix',    color:'#0033FF', m:'58' },
  { who:'@bigben.bets',  txt:'Ousmane !!!! 2-1 FRA !!!! 🇫🇷', color:'#FF6E00', m:'71' },
]

function LivePitch({
  homeColor, awayColor, homeStarters, awayStarters,
}: { homeColor: string; awayColor: string; homeStarters: any[]; awayStarters: any[] }) {
  ...
  return (
    <div style={{ position:'relative', aspectRatio:'5/3', width:'100%',
      ...
      <div style={{ position:'absolute', top:8, left:12, padding:'4px 10px', background:'var(--paper)', border:'1.5px solid var(--ink)', borderRadius:6, fontFamily:'var(--font-archivo-black), Archivo Black', fontSize:12 }}>BRA →</div>
      <div style={{ position:'absolute', top:8, right:12, padding:'4px 10px', background:'var(--paper)', border:'1.5px solid var(--ink)', borderRadius:6, fontFamily:'var(--font-archivo-black), Archivo Black', fontSize:12 }}>← FRA</div>
    </div>
  )
}
...
export function LiveView({ onOpenTeam }: { onOpenTeam?: (code: string) => void }) {
  const home = teamByCode('BRA')
  const away = teamByCode('FRA')
  const homeLineup = LINEUPS.BRA
  ...
  const [odds, setOdds]     = useState({ home: 6.5, draw: 3.1, away: 1.45 })
  ...
  useEffect(() => {
    const phrases = [
      'Belle relance de Marquinhos !','Mbappé toujours dangereux','Casemiro tient bon au milieu',
      'Vini déborde, ça pique','Ekitiké presse haut','Le rythme est fou ce soir',
      'Dembélé !!! il était proche','Allez les Bleus !!! 🇫🇷','Vamos Brasil 🇧🇷',
      'Quel match... 🤯','J’ai mis ma mise sur 2.7, easy money 💸',
      'Sub à venir pour Deschamps ?','Olise au pressing constant 💪',
    ]
  ...
  return (
    <section style={{ background:'var(--paper)' }}>
      <div style={{ background:'var(--ink)', color:'var(--paper)', borderBottom:'1.5px solid var(--ink)' }}>
        <div style={{ maxWidth:1320, margin:'0 auto', padding:'18px 32px', display:'grid', gridTemplateColumns:'auto 1fr auto', alignItems:'center', gap:24 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:24 }}>
            <div onClick={() => onOpenTeam && onOpenTeam('BRA')} style={{ display:'flex', alignItems:'center', gap:14, cursor:'pointer' }}>
              <Flag team={home} w={48} h={32}/>
              <span className="display" style={{ fontSize:30 }}>BRA</span>
            </div>
            ...
            <div onClick={() => onOpenTeam && onOpenTeam('FRA')} style={{ display:'flex', alignItems:'center', gap:14, cursor:'pointer' }}>
              <span className="display" style={{ fontSize:30 }}>FRA</span>
          ...
          <div style={{ textAlign:'right', fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.6)', letterSpacing:'0.04em' }}>
            <div>AT&amp;T STADIUM · DALLAS</div>
            <div style={{ marginTop:2 }}>JOURNÉE 1 · GROUPE F</div>
          </div>
        </div>
        ...
        <div style={{ maxWidth:1320, margin:'0 auto', padding:'0 32px 16px' }}>
          <div style={{ display:'grid', gridTemplateColumns:'auto 1fr auto', alignItems:'center', gap:14, fontSize:11, color:'rgba(255,255,255,0.7)', fontWeight:700, letterSpacing:'0.06em' }}>
            <span style={{ color: PALETTE.lime }}>BRA · {possession.toFixed(0)}%</span>
            ...
            <span style={{ color: PALETTE.lime, textAlign:'right' }}>FRA · {(100-possession).toFixed(0)}%</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:1320, margin:'0 auto', padding:'24px 32px 64px' }}>
        <div style={{ display:'grid', gridTemplateColumns:'1.6fr 1fr', gap:18, marginBottom:18 }}>
          <div className="card" style={{ padding:18 }}>
            ...
            <LivePitch homeColor={home.color} awayColor={away.color}
            ...
            <div style={{ marginTop:14, padding:'12px 16px', background: PALETTE.lime, borderRadius:10, display:'flex', alignItems:'center', gap:12 }}>
              ...
              <span style={{ fontSize:13, fontWeight:800 }}>⚽ BUT — Ousmane Dembélé (FRA) — contre-attaque conclue à 8 mètres</span>
            </div>
          </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1.6fr 1fr', gap:18, marginBottom:18 }}>
          ...
          <LiveEventsFeed events={events} home={home} away={away}/>
        </div>
...
function LiveEventsFeed({ events, home, away }: { events: any[]; home: any; away: any }) {
  ...
  return (
    <div className="card" style={{ padding:0, overflow:'hidden' }}>
      ...
      <div style={{ maxHeight:300, overflowY:'auto' }}>
        {sorted.map((e, i) => {
          const tm = e.team === 'BRA' ? home : e.team === 'FRA' ? away : null
          return (
            <div key={i} style={{
  ...
  return (
    <div className="card" style={{ padding:20 }}>
      <div style={{ display:'flex', alignItems:'baseline', justifyContent:'space-between', marginBottom:14 }}>
        ...
        <span style={{ fontSize:10, fontWeight:800, color:'var(--muted)', letterSpacing:'0.06em' }}>BRA vs FRA · 72&apos;</span>
      </div>
...
function LiveBettingPanel({
  ...
  const markets: Record<string, { lbl: string; opts: Array<[string, number]> }> = {
    next_goal:   { lbl:'Prochain but',     opts:[['BRA', 2.1], ['Aucun', 4.0], ['FRA', 1.75]] },
    next_corner: { lbl:'Prochain corner',  opts:[['BRA', 1.95], ['Aucun', 5.5], ['FRA', 1.95]] },
    over_total:  { lbl:'Total buts > 3.5', opts:[['Oui', 2.6], ['Non', 1.45]] },
    scorer:      { lbl:'Prochain buteur',  opts:[['Mbappé', 4.5], ['Vini', 5.0], ['Endrick', 6.0]] },
  }
